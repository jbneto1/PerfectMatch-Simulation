...
begin
  ...
  begin
    resetRobot();
    currentTarget.x := -0.57; //-0.555
    currentTarget.y := 0.235; //0.245
    currentTarget.angle := rad(92);
  ...
  begin
    currentTarget.x := -0.01;
    currentTarget.y := -0.277;
    currentTarget.angle := rad(-90);
...
begin
  ...
  try
    ...
    WriteUDPData('172.27.127.231', 9000, StrPacket.text);
...
begin
  ...
  if finish then
  begin
    saveToFile('D:\Clouds\OneDrive - Universidade do Porto\Documents\GitHub\PainAndSuffering\feedback\trajectory.txt');
    finish := False;
  end;
