...
uses
  ...
  IniPropStorage, BGRABitmap, BGRABitmapTypes, math, utils, laserLoc;
...
begin
  ...
  if ssCtrl in Shift then begin
    RobotPose.x := mouse_x;
    RobotPose.y := mouse_y;
    RobotPose.theta := degtoRad(0);
    //DrawRobot(R);
  end else begin
    //bmp.DrawLine(x, y, x - round(GradXMap[y, x] * 10000), y - round(GradYMap[y, x] * 10000), BGRA(255, 0, 0, 128), true);
    //for i := 0 to 10 do
    //  IterLaser(RobotPose, FLaserLoc.LaserPoints, laserFirstIdx, laserLastIdx, 0.01);
  end;
...
begin
  ...
  if ssCtrl in Shift then begin
    xw := pixelToX(x);
    yw := pixelToY(y);
    RobotPose.theta := atan2(yw -RobotPose.y, xw  - RobotPose.x);
  end;
...
begin
  ...
  for i := FirstIdx to LastIdx do begin
    ...
    if (u > 0) and (u < ImgWidth -1) and
      ...
      dx := dx - GradX / LaserPoints[i].std;
      dy := dy + GradY / LaserPoints[i].std;
      dtheta := dtheta - GradX / LaserPoints[i].std * ( - LaserPoints[i].x * st - LaserPoints[i].y * ct)
