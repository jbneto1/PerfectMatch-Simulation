...
def send_yolo_data(yolo_data):
    try:
        yolo_sock.sendto(
            yolo_data.encode(), (ip_wsl2, port_yoloMsg)
        )  # Sending to the C++ application
    except Exception as e:
    ...
    while running:
        try:
            ...
            for result in results:
                ...
                boxes = (
                    result.boxes.xywh
                )  # Assuming this is a tensor of shape [N, 4] where N is the number of boxes
                ...
                if len(classes) != 0:
                    ...
                    for i in range(len(classes)):
                        x1, y1, w, h = boxes[
                            i
                        ].tolist()  # Correctly indexing the i-th box
                        conf = confidences[
                            i
                        ].item()  # Correctly indexing the i-th confidence, converting to Python scalar
                        cls = int(
                            classes[i].item()
                        )  # Correctly indexing the i-th class, converting to Python scalar
