...
file_path_semantics = "docs/logs/logs_offlineAnalysis/offlineAnalysis_w_semantics2024-02-27_16-08-05.txt"
#%%
#Case STUDY II
file_path = 'docs/logs/logs_offlineAnalysis/offlineAnalysis_2024-02-29_12-19-56.txt'
file_path_semantics = "docs/logs/logs_offlineAnalysis/offlineAnalysis_w_semantics2024-02-29_12-19-56.txt"
#%%
...
def transform_dataframe_to_mm(df):
    ...
    cov_with_theta = ['EKFCovXTheta', 'EKFCovYTheta', 'EKFCovThetaX', 'EKFCovThetaY', 'EKFCovThetaTheta']
...
def draw_orientation_arrow(ax, x, y, theta, length=80, color='r'):
    ...
    # Draw the arrow
    ax.arrow(x, y, end_x - x, end_y - y, head_width=length*0.15, head_length=length*0.15, fc=color, ec=color)
...
for i in range(0, len(df), decimation_factor):
    ...
    ellipse = draw_cov_ellipse(cov_matrix, (df.iloc[i]['EKF_x'], df.iloc[i]['EKF_y']), nstd=1, ax=axs[0], alpha=alpha, color='purple', label='1 STD confidence ellipse' if i == 0 else "")
    axs[0].scatter(gt_pose_x, gt_pose_y, color='black', marker='x', label='Ground truth trajectory' if i == 0 else "")

for i in range(0, len(df), decimation_factor_arrows):
    draw_orientation_arrow(axs[0], df.iloc[i]['EKF_x'], df.iloc[i]['EKF_y'], df.iloc[i]['EKF_theta'], color='green')
    
axs[0].plot(df['EKF_x'], df['EKF_y'], label='EKF pose trajectory', color=color_df_wo_semantics)
...
for i in range(0, len(df_semantics), decimation_factor):
    ...
    ellipse = draw_cov_ellipse(cov_matrix, (df_semantics.iloc[i]['EKF_x'], df_semantics.iloc[i]['EKF_y']), nstd=1, ax=axs[1], alpha=alpha, color='blue', label='1 STD confidence ellipse' if i == 0 else "")
    axs[1].scatter(gt_pose_x, gt_pose_y, color='black', marker='x', label='Ground truth trajectory' if i == 0 else "")

for i in range(0, len(df_semantics), decimation_factor_arrows):
    draw_orientation_arrow(axs[1], df_semantics.iloc[i]['EKF_x'], df_semantics.iloc[i]['EKF_y'], df_semantics.iloc[i]['EKF_theta'], color=color_df_w_semantics)

axs[1].plot(df_semantics['EKF_x'], df_semantics['EKF_y'], label='EKF pose trajectory', color=color_df_w_semantics)
...
abs_errorPM_semantics = df_semantics['errorPM'].abs() if 'errorPM' in df_semantics else pd.Series()
...
plot_with_y_zero(axs[0, 0], abs_errorEKF_x_original, abs_errorEKF_x_semantics, 'Original system', 'Robust PM', 'Absolute Error in X [mm]')
plot_with_y_zero(axs[0, 1], abs_errorEKF_y_original, abs_errorEKF_y_semantics, 'Original system', 'Robust PM', 'Absolute Error in Y [mm]')
plot_with_y_zero(axs[1, 0], abs_errorEKF_theta_original, abs_errorEKF_theta_semantics, 'Original system', 'Robust PM', 'Absolute Error in θ [rad]')

if not abs_errorPM_original.empty and not abs_errorPM_semantics.empty:
    plot_with_y_zero(axs[1, 1], abs_errorPM_original, abs_errorPM_semantics, 'Original system', 'Robust PM', 'Absolute Error in PM [mm]')
else:
    axs[1, 1].clear()
...
plt.plot(df_semantics['EKF_x'], df_semantics['EKF_y'], label='Robust PM', linestyle='-', color='green')
...
#FIXME: the errors should be in absolute values before applying metrics
...
def to_latex_custom(df, decimals, num_format_dec, caption, label, per=False):
    ...
    formatted_df = df.apply(lambda x: x.round(decimals).apply(lambda y: num_format.format(y)))

    # Generate the LaTeX code for the table
    latex_str = formatted_df.to_latex(index=True, column_format="l" + "r" * (len(df.columns)), escape=False)
...
latex_table3 = to_latex_custom(error_percentage, decimals, num_format, caption3, label3, True)
print(latex_table1)
print(latex_table2)
print(latex_table3)
