...
MecanumKinematics::MecanumKinematics(const double a, const double b, const double r, const double dt) :
        a(a), b(b), r(r), dt(dt),
...
Eigen::Vector4d MecanumKinematics::estimateWheelSpeeds(const std::array<int, 4> &encoders) {
    ...
    if (encoders.size() != 4) std::runtime_error("Encoders size incorrect.");
